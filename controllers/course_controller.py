...
from database.models import Course, Section, UserSection    
...
def get_section(section_id: str, course_id: str):
    section = session.query(Section).filter(Section.id == section_id, Section.course_id == course_id).first()
...
def add_user_section(id: str, user_id: str, section_id: str):
    user_course = UserSection(user_id=user_id, course_id=section_id)
